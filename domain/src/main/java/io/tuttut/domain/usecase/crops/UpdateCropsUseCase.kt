...
class UpdateCropsUseCase @Inject constructor(
    private val cropsRepository: CropsRepository,
    private val preferenceRepository: PreferenceRepository,
) {
    suspend operator fun invoke(updateCropsRequest: UpdateCropsRequest): Result<String> = runCatchingExceptCancel {
        val credential = preferenceRepository.getCredentialFlow().first()
        cropsRepository.updateCrops(credential.gardenId, updateCropsRequest)
    }
}
